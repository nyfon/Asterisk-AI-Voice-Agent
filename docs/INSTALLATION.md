# Asterisk AI Voice Agent - Installation Guide (v6.5.1)

This guide provides detailed instructions for setting up the Asterisk AI Voice Agent v6.5.1 on your server.

## Three Setup Paths

Choose the path that best fits your experience level:

## Upgrade to v6.5.1 (Existing Checkout)
...
To upgrade to the tagged `v6.5.1` release (once the tag is published):
...
> Note: `agent quickstart` and `agent init` are still available for backward compatibility, but `agent setup` is the recommended CLI wizard for v6.5.1.
